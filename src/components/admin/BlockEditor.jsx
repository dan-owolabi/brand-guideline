...
export default function BlockEditor({ block, brand, onUpdate, onDelete }) {
...
// Helpers
const LabelWithDesc = ({ label, desc }) => (
...
// Individual Editors
function HeroEditor({ data, onChange, onImageUpload }) {
    return (
...
function DoDontEditor({ data, onChange }) {
    const items = data.items || []
    const addItem = (type) => onChange('items', [...items, { type, description: '' }])
...
function LogoShowcaseEditor({ data, onChange, onImageUpload }) {
    const logos = data.logos || []
    const addLogo = () => onChange('logos', [...logos, { name: 'New Variant', imageUrl: '', description: '' }])
    const updateLogo = (idx, fld, val) => {
        const list = [...logos]; list[idx] = { ...list[idx], [fld]: val }; onChange('logos', list)
    }
    const removeLogo = (idx) => onChange('logos', logos.filter((_, i) => i !== idx))
...
function TypographyEditor({ data, onChange }) {
    ...
    const updateSpec = (idx, fld, val) => {
        const list = [...specimens]; list[idx] = { ...list[idx], [fld]: val }; onChange('specimens', list)
    }
    const removeSpec = (idx) => onChange('specimens', specimens.filter((_, i) => i !== idx))
...
function AssetGridEditor({ data, onChange, onImageUpload }) {
    const assets = data.assets || []
    const addAsset = () => onChange('assets', [...assets, { name: 'Asset Name', description: '', thumbnailUrl: '', downloadUrl: '' }])
    const updateAsset = (idx, fld, val) => {
        const list = [...assets]; list[idx] = { ...list[idx], [fld]: val }; onChange('assets', list)
    }
    const removeAsset = (idx) => onChange('assets', assets.filter((_, i) => i !== idx))

    const handleUpload = async (idx, file, field) => {
        try {
            const url = await uploadFile(file)
