...
const SheetTrigger = ({ children, asChild, setOpen, className }) => {
...
const SheetContent = ({ children, open, setOpen, side = "left", className }) => {
    const sheetRef = React.useRef(null)

    React.useEffect(() => {
        const handleClickOutside = (event) => {
            if (open && sheetRef.current && !sheetRef.current.contains(event.target)) {
                setOpen(false)
            }
        }
