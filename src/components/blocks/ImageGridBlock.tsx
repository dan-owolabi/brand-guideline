...
import { ImagePlus, X, Plus, Grid3x3, Square, RectangleHorizontal, RectangleVertical, Maximize2 } from 'lucide-react'
...
interface ImageGridBlockProps {
    content: ImageGridBlockContent
    isAdmin?: boolean
    onUpdate?: (content: ImageGridBlockContent) => void
}
            ...
            <div className={`my-6 grid ${gridCols} gap-6`}>
                {images.map((img, index) => (
                    <figure key={index} className="relative">
                        <div className={`relative w-full ${!isAutoAspect ? aspectClass : 'aspect-square'} overflow-hidden rounded-lg`}>
                            <Image
                                src={img.src}
                                alt={img.alt || ''}
                                fill
