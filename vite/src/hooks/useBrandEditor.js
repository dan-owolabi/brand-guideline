    ...
    useEffect(() => {
        ...
        const fetchDraft = async () => {
            ...
            try {
                ...
                // Seed missing/empty drafts in local state only. Persisting here would
                // make a read mutate the DB (and fail under viewer RLS); the first user
                // edit saves the seeded content via the debounced writer instead.
                if (!existingDraft.sections || existingDraft.sections.length === 0) {
                    ...
                    finalDraft = publishedUsable ? data.published : getDefaultDraft()
