...
export default function PublicBrandApp({ brandIdentifier, isCustomDomain = false }) {
    ...
    const loadBrand = useCallback(async () => {
        try {
            ...
            // tables. The views expose an explicit allowlist, so `draft` and
            // `billing_email` are not reachable even via ?select=.
            // See supabase/migrations/010_public_views_column_scope.sql.
            let query = supabase
                .from('public_brands')
