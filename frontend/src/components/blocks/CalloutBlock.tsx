...
import { AlertCircle, Info, Lightbulb, AlertTriangle, LucideIcon } from 'lucide-react'
...
interface CalloutBlockProps {
    ...
    isAdmin?: boolean
    onUpdate?: (content: CalloutBlockContent) => void
}
        ...
        <div className={`${style.bg} ${style.border} border rounded-lg p-4 flex gap-3 my-4`}>
            ...
            <div className="flex-1">
                {isAdmin && isEditing ? (
                    ...
                    >
                        {localText || (isAdmin ? <span className="opacity-50">Click to add message...</span> : '')}
                    </p>
                )}
            </div>
        </div>
